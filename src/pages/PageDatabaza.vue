...
export default {
	...
	methods: {
		...
		reloadData() {
			...
			axios.post(`${process.env.SERVER_URL}/api/databaza/get-contacts`, {
		...
		saveContact() {	
			Loading.show({ spinner: QSpinnerGears })
			axios.post(`${process.env.SERVER_URL}/api/databaza/set-contact`, {
		...
		deleteContact() {
			Loading.show({ spinner: QSpinnerGears })		
			axios.post(`${process.env.SERVER_URL}/api/databaza/delete-contact`, {
